...
public class Main {
    ...
    public static int countNodes(BNode<String> node,int depth){
      if(node==null){
          return 0;
      }
      if(node.left==null && node.right==null){
     ...
     return Math.max( countNodes(node.left,depth+1),countNodes(node.right,depth+1));
    }
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<n+m;i++){
            ...
            if(input[0].equals("root")){
                drvo.makeRoot(input[1]);
            }else if(input[0].equals("add")){
                if(input[3].equals("LEFT")) {
                    drvo.addChild(drvo.find(input[1]),1,input[2]);
                }else{
                    drvo.addChild(drvo.find(input[1]),2,input[2]);
                }
            }else{
                System.out.println(countNodes(drvo.find(input[1]),1));
            }
        }
