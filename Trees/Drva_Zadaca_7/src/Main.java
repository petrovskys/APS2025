...
public class Main {
    ...
    public static int nodeDepth(int k, BNode<Integer> node) {
        if (node == null) {
            return 0;
        }
        if(node.info<k)return 1+nodeDepth(k,node.left)+nodeDepth(k,node.right);
        else return nodeDepth(k,node.left);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n + m; i++) {
            ...
            if (input[0].equals("insert")) {
                ...
                System.out.println(nodeDepth(Integer.parseInt(input[1]), tree.getRoot()));
            }
        }
    }
}
