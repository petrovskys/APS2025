...
public class Main {
    ...
    public static int nodeDepth(int k,BNode<Integer> node,int depth){
        if(node==null){
            return 0;
        }
        if(node.info.equals(k)){
            return depth;
        }else if(k<node.info){
            return nodeDepth(k,node.left,depth+1);
        }else return nodeDepth(k,node.right,depth+1);
    }
    public static void main(String[] args){
        ...
        for(int i=0;i<n+m;i++){
            ...
            if(input[0].equals("insert")){
                ...
                System.out.println(nodeDepth(Integer.parseInt(input[1]),tree.getRoot(),1));
            }
        }
    }
}
